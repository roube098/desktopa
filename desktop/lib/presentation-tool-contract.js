const DEFAULT_EDITOR_BRIDGE_TIMEOUT_MS = 8000;
const PRESENTATION_STEADY_STATE_TIMEOUT_MS = 12000;
const PRESENTATION_COLD_START_TIMEOUT_MS = 30000;

function isPresentationContext(toolCall = {}, runtimeContext = {}) {
...
function isPresentationColdStartContext(runtimeContext = {}) {
...
function getOnlyOfficeToolTimeoutMs(toolCall = {}, runtimeContext = {}) {
...
function buildOnlyOfficeToolTimeoutResult(toolCall = {}, runtimeContext = {}, timeoutMs = DEFAULT_EDITOR_BRIDGE_TIMEOUT_MS) {
...
function normalizeOnlyOfficeToolResult(rawResult, toolCall = {}, runtimeContext = {}) {
  const input = rawResult && typeof rawResult === "object" ? rawResult : {};
  const inputData =
  ...
  if (normalized.success && toolCall.toolName === "addSlide") {
    const firstResult = Array.isArray(normalized.data.results) ? normalized.data.results[0] : null;
    if (firstResult && Number.isInteger(firstResult.slideIndex) && !Number.isInteger(normalized.data.slideIndex)) {
      normalized.data.slideIndex = firstResult.slideIndex;
    }
  }
