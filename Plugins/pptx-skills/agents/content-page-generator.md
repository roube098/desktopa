...
### 7. Market narrative (Adoption / S-curve)
- Headline stat + subtitle, central S-curve with “current vs future” markers, four-column footer (narrative + stat pairs)
- **Do not implement inline** — delegate to **`market-narrative-page-generator`** so curve geometry, markers, and footer grid stay consistent
- Layout reference:
  ```
  |  $52B market.                         |
  |  No execution-layer winner yet.        |
  |                     ╭── S-curve ──╮   |
  |                     ●  ○            |
  |  [para]  [stat]  [stat]  [stat]     |
  ```

### 8. Dashboard charts (default style for any chart slide)
- Pie or doughnut, pill-shaped tags, capsule column comparisons, overlapping triple-column groups — **same visual language on light, dark, or brand** palettes (dark+lime is one documented preset)
- **Delegate to `dashboard-chart-generator`** — shape-based “pill” bars and stepped overlaps are easy to get wrong if you only use default chart presets

### Investment research (when outline calls for narrative, not primary tables)

Use **investment-research-skill** for deck-level rules. For content slides that remain **narrative** (thesis pillars, risks as bullets, market story):

- **Title** = takeaway (not “Industry overview”).
- Include **Source:** line if any number or market claim appears.
- Prefer **Sharp & Compact** density; avoid decorative underlines under titles.
- If the slide is **mostly a table or KPI row**, use **financial-data-page-generator** instead.
